# updateEngine
# versionName = 1.1.2
# Updater system OTA to ESP32 modules from github repository
#
...
#### -------------User Variables----------------####
user = 'UserAccountNotFound'
repository = 'ESP32-Y61-HFF'
gitflow = 'main'
token = ''
# Change this variable to 'master' or any other name matching your default branch
gitflow_updater = 'main'
# Don't remove sysUpdate.py from the ignore_files unless you know what you are doing :D
# Put the files you don't want deleted or updated here use '/filename.ext'
ignore_files = ['/systemUpdate.py', '/envUser.conf']
ignore = ignore_files

moduleName = 'HeatFuelFilter'
### -----------END OF USER VARIABLES ----------####

# Static URLS
# GitHub uses 'main' instead of master for python repository trees
giturl = 'https://github.com/{user}/{repository}'
call_trees_url = f'https://api.github.com/repos/{user}/{repository}/git/trees/{gitflow}?recursive=1'
raw = f'https://raw.githubusercontent.com/{user}/{repository}/{gitflow}/'

def pull(f_path,raw_url):
  print(f'pulling {f_path} from github')
  #files = os.listdir()
  headers = {'User-Agent': 'updateEngine-ESP32-module'} 
...
def pull_all(tree=call_trees_url,raw = raw,ignore = ignore,isconnected=False):
  ...
  logfile = open('update_system_log.log','w')
...
def wificonnect(ssid=ssid,password=password):
    network.hostname(moduleName)
    print('Use: like sysUpdate.wificonnect(SSID,Password)')
    print('otherwise uses ssid,password in top of envUser.py')
...
def pull_git_tree(tree_url=call_trees_url,raw = raw):
  ...
  if 'tree' not in data:
      print('\nDefault branch "main" not found. Set "branch" variable to your default branch.\n')
      raise Exception(f'Default branch {branch} not found.') 
...
def update():
    print('updates systemUpdate.py to newest version')
    raw_url = 'https://raw.githubusercontent.com/UserAccountNotFound/ESP-OTA-GIT/{gitflow}/'
    pull('systemUpdate.py', raw_url + 'systemUpdate.py')

def backup():
    int_tree = build_internal_tree()
    backup_text = "updateEngine Backup Version 1.0\n\n"
    for i in int_tree:
        data = open(i[0],'r')
        backup_text += f'FN:SHA1{i[0]},{i[1]}\n'
        backup_text += '---'+data.read()+'---\n'
        data.close()
    backup = open('updateEngine.backup','w')
